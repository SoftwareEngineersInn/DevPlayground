...
compatibility: NodeJS, GitHub PAT authorized to cisco-genai org, APM or permission to install it, and access to the target repository
metadata:
  version: "3.1"
---
...
- Resolve the latest published package tag at install time. Use `git ls-remote --tags https://github.com/cisco-genai/awesome-cisco "<name>-v*"`, sort the returned tags by semver, and install the newest matching tag. Do not hardcode package versions in this skill.
- Use the detected IDE targets in every package install command and include protocol fallback:

```bash
apm install --target <ide-1> --target <ide-2> --allow-protocol-fallback cisco-genai/awesome-cisco/packages/<name>#<name>-v<latest>
```
...
| Atmosphere or Harbor | `atmosphere` |
...
- Browse `awesome-cisco` first for Cisco-curated skills that match the detected stack.
- Use `npx skills find <query>` only as a search mechanism when the needed skill is not already available through repo-local sources or `awesome-cisco`.
...
- Treat package-provided MCP servers as already handled by the corresponding package install:
  ...
  - `atmosphere` provides the CDSI Atmosphere MCP server.

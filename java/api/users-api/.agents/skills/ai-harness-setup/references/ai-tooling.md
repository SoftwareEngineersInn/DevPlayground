...
- If APM needs access to private GitHub sources such as `awesome-cisco`, make sure `GITHUB_CLI_PAT` or `GITHUB_TOKEN` is available.
- Use `npx skills find` only for broader discovery when repo-local sources and `awesome-cisco` do not already cover the need.
...
3. Resolve the latest published tag for that package with `git ls-remote --tags https://github.com/cisco-genai/awesome-cisco "<name>-v*"` and install the newest matching tag.
...
- Browse `awesome-cisco` first for curated skills that match the detected stack.
- Use `npx skills find <query>` only as a search mechanism when a needed skill is not already available through repo-local sources or `awesome-cisco`.

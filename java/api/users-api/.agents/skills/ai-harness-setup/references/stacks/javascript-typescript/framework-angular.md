...
```bash
apm install --target <ide-1> --target <ide-2> --allow-protocol-fallback cisco-genai/awesome-cisco/packages/stack-angular#stack-angular-v<latest>
```
...
Search for additional Angular-specific review or frontend testing skills in local sources, `awesome-cisco`, and the broader skills ecosystem only when the package does not cover the repo's needs, then install the selected package with `apm install`.

...
Review repo-local skills and the `awesome-cisco` skill inventory first, then use `npx skills find` (or `bunx skills find` for bun repos) only when broader discovery is still needed. Install selected skills with `apm install`.
...
```bash
apm install --target <ide-1> --target <ide-2> --allow-protocol-fallback cisco-genai/awesome-cisco/packages/stack-javascript-typescript#stack-javascript-typescript-v<latest>
```

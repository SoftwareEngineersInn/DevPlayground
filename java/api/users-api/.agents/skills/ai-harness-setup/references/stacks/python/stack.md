...
```bash
apm install --target <ide-1> --target <ide-2> --allow-protocol-fallback cisco-genai/awesome-cisco/packages/stack-python-uv#stack-python-uv-v<latest>
```
...
For extra Python-specific needs, browse repo-local skills and `awesome-cisco` first, then use `npx skills find` only when broader discovery is still needed. Install only the additional skills that fill real gaps in the repo's current workflow.

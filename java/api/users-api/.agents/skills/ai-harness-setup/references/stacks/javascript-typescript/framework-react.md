...
```bash
apm install --target <ide-1> --target <ide-2> --allow-protocol-fallback cisco-genai/awesome-cisco/packages/stack-react#stack-react-v<latest>
```
...
Browse `awesome-cisco` or other curated sources first, then use `npx skills find` with queries such as:
